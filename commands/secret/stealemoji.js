...
module.exports = {
 ...
 // Change this if you have a diffrent cmd handler
  ...
  run : async(client , message, args) => {// Change this if you have a diffrent cmd handler 
    ...
    if(!message.member.hasPermission("MANAGE_EMOJIS")) return message.channel.send('คุณไม่มีสิทธิพอน่ะคะ ต้องการยศ [MANAGE_EMOJIS] เพื่อใช้คำสั่งนี้น่ะคะ')

    const emoji = args[0];
    const name = args.slice(1).join(" ");
    if (!emoji) {
      const embed = new MessageEmbed()
               .setDescription(`โปรดระบุอีโมจิที่ต้องการด้วยน่ะคะ`)
               .setColor('RANDOM')
    ...
    try {
      if (emoji.startsWith("https://cdn.discordapp.com")) {
        ...
        const embed = new MessageEmbed()
          .setTitle(`นี่คืิิิออีโมจิของคุณค่ะ`)
          .setThumbnail(`${emoji}`)
          .setColor('#FF69B4')
          .setDescription(
            `ชื่อของอีโมจิ : ${

...
//voice state update event to check joining/leaving channels
  ...
  if (!oldState.channelID && newState.channelID) {
    if(newState.channelID !== await db.get(`voice_${newState.guild.id}_1`)) return;  //if its not the jointocreatechannel skip
  ...
  if (oldState.channelID && newState.channelID) {
    ...
    if (oldState.channelID !== newState.channelID) {
      ...
      if(newState.channelID=== await db.get(`voice_${newState.guild.id}_1`)) 
      //make a new channel
